# Промпт для Claude Code: деплой агента «Отчёт для руководства + HTML-дашборд» в ASCN

Скопируй текст ниже в Claude Code, запущенный в этой папке (13-management-report-published/).

---

Ты переносишь готового агента «Отчёт для руководства + HTML-дашборд» на платформу ASCN (ascn.ai).
...
2. Перенеси в воркспейс агента ASCN:
   ...
   - workspace/skills/kpi-report/SKILL.md → /data/workspace/skills/kpi-report/SKILL.md
   - workspace/skills/html-dashboard/SKILL.md → /data/workspace/skills/html-dashboard/SKILL.md
   - workspace/knowledge/METRICS_MAP.md → /data/workspace/knowledge/METRICS_MAP.md
   - workspace/knowledge/REPORT_STRUCTURE.md → /data/workspace/knowledge/REPORT_STRUCTURE.md
...
5. Напомни мне вручную: при первом диалоге агент сам запросит подключение Google Sheets и Drive (OAuth-кнопки в чате), опционально Search Console и GitHub; канал Slack/Telegram — если нужен.
6. Отправь агенту первое сообщение «Привет, начнём настройку» и убедись, что он начал онбординг по шагам (представился, назвал время настройки ~15 минут, задал первый вопрос про метрики).
