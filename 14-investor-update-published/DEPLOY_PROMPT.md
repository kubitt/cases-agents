# Промпт для Claude Code: деплой агента «Инвестор-апдейт и борд-дек» в ASCN

Скопируй текст ниже в Claude Code, запущенный в этой папке (14-investor-update-published/).

---

Ты переносишь готового агента «Инвестор-апдейт и борд-дек» на платформу ASCN (ascn.ai).
...
2. Перенеси в воркспейс агента ASCN:
   ...
   - workspace/skills/investor-update/SKILL.md → /data/workspace/skills/investor-update/SKILL.md
   - workspace/skills/board-deck-outline/SKILL.md → /data/workspace/skills/board-deck-outline/SKILL.md
   - workspace/knowledge/METRICS_SOURCES.md → /data/workspace/knowledge/METRICS_SOURCES.md
   - workspace/knowledge/UPDATE_TEMPLATE.md → /data/workspace/knowledge/UPDATE_TEMPLATE.md
...
5. Напомни мне вручную: при первом диалоге агент сам запросит подключение Google Sheets и Gmail (OAuth-кнопки в чате), опционально Slides и Drive для борд-дека.
6. Отправь агенту первое сообщение «Привет, начнём настройку» и убедись, что он начал онбординг по шагам (представился, назвал время настройки ~10 минут, задал первый вопрос про стадию и инвесторов).
