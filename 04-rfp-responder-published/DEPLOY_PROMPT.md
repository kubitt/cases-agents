# Промпт для Claude Code: деплой агента «RFP/тендер-ответчик» в ASCN

Скопируй текст ниже в Claude Code, запущенный в этой папке (04-rfp-responder-published/).

---

Ты переносишь готового агента «RFP/тендер-ответчик» на платформу ASCN (ascn.ai).
...
2. Перенеси в воркспейс агента ASCN:
   ...
   - workspace/knowledge/COMPLIANCE_CHECKLIST.md → /data/workspace/knowledge/COMPLIANCE_CHECKLIST.md
   Системные файлы платформы (AGENTS.md, MEMORY_POLICY.md, INTEGRATIONS.md, AUTOMATIONS.md, CHANNELS.md, rules/) НЕ трогай.
3. Автоматизации НЕ создавай сам: их создаст агент на финальном шаге онбординга по файлу ONBOARDING.md (текст опциональной задачи мониторинга лежит в AUTOMATIONS.md этой папки — при желании сверь).
4. Проверь себя: выведи список перенесённых файлов и первые 3 строки каждого из воркспейса агента.
5. Напомни мне вручную: подключить интеграции (Google Drive и Google Docs; по желанию Google Sheets и Gmail) при первом диалоге с агентом — онбординг сам их запросит; подключить канал Telegram, если нужен.
6. Отправь агенту первое сообщение «Привет, начнём настройку» и убедись, что он начал онбординг по шагам (представился, назвал время настройки ~15 минут, задал первый вопрос — чем занимается компания и на какие тендеры отвечаете).
