# Промпт для Claude Code: деплой агента «GEO-оптимизатор» в ASCN

Скопируй текст ниже в Claude Code, запущенный в этой папке (08-geo-optimizer-published/).

---

Ты переносишь готового агента «GEO-оптимизатор» на платформу ASCN (ascn.ai).
...
2. Перенеси в воркспейс агента ASCN:
   ...
   - workspace/skills/geo-audit/SKILL.md → /data/workspace/skills/geo-audit/SKILL.md
   - workspace/skills/geo-content-rewrite/SKILL.md → /data/workspace/skills/geo-content-rewrite/SKILL.md
   - workspace/knowledge/GEO_CHECKLIST.md → /data/workspace/knowledge/GEO_CHECKLIST.md
   - workspace/knowledge/BRAND_FACTS.md → /data/workspace/knowledge/BRAND_FACTS.md
...
5. Напомни мне вручную: подключить интеграции (Google Docs, опционально Google Search Console и Google Sheets) при первом диалоге с агентом — онбординг сам их запросит; подключить канал Telegram, если нужен.
6. Отправь агенту первое сообщение «Привет, начнём настройку» и убедись, что он начал онбординг по шагам (представился, назвал время настройки ~10 минут, задал первый вопрос про сайт и нишу).
